...
export function Checkbox(props: CheckboxProps) {
  return (
    <div className={clsx('flex align-center gap-2',{'text-gray-200': props.disabled ===false, 'text-gray-600': props.disabled === true})}>
        <CheckboxPrimitive.Root className='w-6 h-6 p-[2px] bg-gray-700 rounded' disabled={props.disabled} defaultChecked={props.checked}>
        ...
        <Text size='sm' color={props.disabled ? 'text-gray-400': 'text-gray-100'}>{props.label}</Text>
    </div>
  )
}
